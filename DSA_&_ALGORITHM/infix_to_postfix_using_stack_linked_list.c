...
Node * infix_to_postfix(Node *infix){
    ...
    while(infix != NULL){
        if(!(infix->data == '+' || infix->data == '-' || infix->data == '*' || infix->data == '/')){
            postfix = insert_at_first(postfix, infix->data);
            infix = infix->next;
        }
        else{

            //----------------- Alternative Code for the else portion ---------------------//
            // while(!isempty(store) && precedence(infix->data) <= precedence(top(store))){
            //     postfix = insert_at_first(postfix, pop(store));

            // }
            // push(store,infix->data);
            // infix = infix->next;


            if(precedence(infix->data) > precedence(top(store))){
                push(store,infix->data);
                infix = infix->next;
            }
            else{
...
int main(){
    ...
    display_linked_list(infix);
    postfix = infix_to_postfix(infix);
    printf("Your postfix expression : ");
    display_linked_list(postfix);
